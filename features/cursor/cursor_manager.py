...
surveillance du premier plan pour activer/désactiver le curseur Roblox
...
Le crosshair, LUI, ne suit PAS ce déclencheur (demande explicite : visible
en permanence — bureau, n'importe quel jeu, même au-dessus des pages de
Zenkai Core — pas seulement Roblox au premier plan) : dès qu'un chemin est
configuré (crosshair_path non vide), l'overlay reste affiché en continu,
indépendamment de _watch_tick/_ROBLOX_PROCESS_NAMES ci-dessous (qui ne
pilotent donc plus QUE le curseur Roblox). Sa fenêtre (voir
crosshair_overlay.py) est déjà WindowStaysOnTopHint + click-through, donc
rester affichée en permanence ne gêne jamais l'app elle-même ni le jeu en
dessous.
"""
import ctypes
...
class CursorManager(QObject):
    def __init__(self, parent=None):
        ...
        # Le timer de surveillance du premier plan ne pilote plus que le
        # curseur Roblox (voir docstring du module) : le crosshair, affiché
        # en continu dès qu'il est configuré, n'a plus besoin de lui.
        if self.roblox_path:
    ...
    def apply_crosshair(self, path: str) -> bool:
        ...
        # Affiché tout de suite, en continu (voir docstring du module) —
        # ne dépend plus du timer de surveillance du premier plan Roblox
        # (celui-ci ne pilote plus que le curseur Roblox).
        self._crosshair_overlay.show_overlay()
        return True
    ...
    def reset_crosshair(self) -> None:
        ...
        # Ne touche jamais _watch_timer : il ne pilote plus que le curseur
        # Roblox désormais (voir docstring du module), indépendant du
        # crosshair.
    ...
    # ------------------------------------------------------------------
    # Surveillance du premier plan (Roblox) — pilote UNIQUEMENT l'overlay
    # curseur Roblox (le crosshair reste affiché en continu dès qu'il est
    # configuré, voir docstring du module et apply_crosshair/reset_crosshair
    # ci-dessus, indépendant de ce déclencheur).
    # ------------------------------------------------------------------
